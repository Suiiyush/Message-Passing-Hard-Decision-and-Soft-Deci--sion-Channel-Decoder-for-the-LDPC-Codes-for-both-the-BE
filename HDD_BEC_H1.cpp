...
int main()
{
	int N=5056,U=3792,i,j,k,count,c1,c2,c3,flag=1,terminate=1,Nsim=1000,Ksim,l,stop;             // N is number of columns and U is number of rows of H - matrix
	...
	while(flag!=0 && terminate!=U && stop<100)
	{
	...
	for(i=0;i<U;i++)
	{
		...
		// If out of 4 VNs connected with a single CN there is not any erasure then, value of terminate will increase and if terminate's value reach to U (means equals to number of CNs), it means that in all Vns there is not any erasure and this is the third breaking condition
	}
