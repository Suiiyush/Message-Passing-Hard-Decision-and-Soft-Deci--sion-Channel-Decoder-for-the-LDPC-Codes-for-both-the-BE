...
int main()
{
	int N=5000,U=3000,i,j,k,count,c1,c2,c3,v1,v2,v3,v4,v5,flag=1,terminate=1,Nsim=1000,Ksim,l,stop;   // N is number of columns and U is number of rows of H - matrix
	...
	srand (time(NULL));
	for(l=0;l<81;l++)                                     // Outer loop for crossover probability
	{
	...
	for(i=0;i<N;i++)
	{
		...
		if(noise[i]==tr[i])                               // Same XOR operation (if r > 1-p then, bit will be flipped)
		rx[i]=0;
		else
		rx[i]=1;
	...
	for(i=0;i<U;i++)
	{
		...
		if((arr_CN[i].v[0][1] + arr_CN[i].v[1][1] + arr_CN[i].v[2][1] + arr_CN[i].v[3][1] + arr_CN[i].v[4][1])%2==0)    // Every CN is checking for even parity
		++stop;
	    ...
	    for(j=0;j<3;j++)
	    {
	    	...
	    	{
	    		arr_VN[v1-1].c[j][1]=(arr_CN[i].v[1][1] + arr_CN[i].v[2][1] + arr_CN[i].v[3][1] + arr_CN[i].v[4][1])%2;
	    		break;
			}
		}
		...
		for(j=0;j<3;j++)
	    {
	    	...
	    	{
	    		arr_VN[v2-1].c[j][1]=(arr_CN[i].v[0][1] + arr_CN[i].v[2][1] + arr_CN[i].v[3][1] + arr_CN[i].v[4][1])%2;
	    		break;
			}
		}
		...
		for(j=0;j<3;j++)
	    {
	    	...
	    	{
	    		arr_VN[v3-1].c[j][1]=(arr_CN[i].v[0][1] + arr_CN[i].v[1][1] + arr_CN[i].v[3][1] + arr_CN[i].v[4][1])%2;
	    		break;
			}
		}
		...
		for(j=0;j<3;j++)
	    {
	    	...
	    	{
	    		arr_VN[v4-1].c[j][1]=(arr_CN[i].v[0][1] + arr_CN[i].v[1][1] + arr_CN[i].v[2][1] + arr_CN[i].v[4][1])%2;
	    		break;
			}
		}
		...
		for(j=0;j<3;j++)
	    {
	    	...
	    	{
	    		arr_VN[v5-1].c[j][1]=(arr_CN[i].v[0][1] + arr_CN[i].v[1][1] + arr_CN[i].v[2][1] + arr_CN[i].v[3][1])%2;
	    		break;
			}
		}
	}
	...
	if(stop==U)    // Because all CNs have even parity and therefore no meaning to decode again
	break;
